  ...
  const handleSubmit = async (event?: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        method: 'createScholarshipProgram',
        args: {
          scoreThreshold: BigInt(score),
          incomeThreshold: BigInt(income),
          ageThreshold: BigInt(age)
        }
      };

      // For now, we'll submit to the backend for storage
      ...
      if (!response.ok) {
        setStatus('error');
        setError('Configuration submission failed. Please check your connection.');
        return;
      }
      ...
      setStatus('success');
      setMessage('Program configuration submitted successfully.');
      setForm({ scoreThreshold: '', incomeThreshold: '', ageThreshold: '' });

      // Refresh state
      const refreshResponse = await fetch(`${indexerUrl}/contracts/${contractAddress}`);
