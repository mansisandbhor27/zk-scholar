...
export default function EligibilityForm() {
  ...
  const handleSubmit = async (event?: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setStatus('error');
        setError('Proof submission failed. Please check your connection.');
        return;
      }
      ...
      setStatus('success');
      setMessage(`Your privacy-preserving proof has been submitted successfully.`);
              ...
              {status === 'success' && (
                <div className="mt-4 p-4 bg-green-900/30 border border-green-600 rounded-lg">
                  ...
                  <div className="mt-2 text-xs text-gray-400">
                    Proof: Submitted • Privacy: Protected • Note: Proof uses local hash for demonstration
                  </div>
                </div>
              )}
          ...
          <div className="glass p-4 rounded-xl">
            ...
            <div className="text-xs text-gray-400 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span>Private inputs stay local</span>
              </div>
              <div className="flex items-center gap-2">
                <Sparkles size={12} className="text-[var(--cyan-glow)]" />
                <span>ZK proof computed locally</span>
              </div>
              <div className="flex items-center gap-2">
                <Database size={12} className="text-[var(--cyan-glow)]" />
                <span>Only proof hash sent to backend</span>
              </div>
            </div>
          </div>

          <div className="glass p-4 rounded-xl">
            <div className="text-sm text-gray-300 mb-2">Recent Proofs</div>
            <div className="text-xs text-gray-400">
              Proof artifacts are stored in your browser's localStorage.
