...
console.log('PROGRAM CREATED:', contractLedger.programCreated);
  ...
  const handleSubmit = async (event?: React.FormEvent) => {
    ...
    console.log('[1] Eligibility submit clicked');
    console.log('[2] Input values', {
      score: form.score,
      income: form.income,
      age: form.dob,
    });
    console.log('[3] callTx exists', Boolean(callTx));
    ...
    // Compact Uint<64> arguments must be non-negative bigint values.
    if (![form.score, form.income, form.dob].every((value) => /^\d+$/.test(value))) {
      setError('Score, income, and age must be whole non-negative numbers.');
      return;
    }

    const score = BigInt(form.score);
    const income = BigInt(form.income);
    const age = BigInt(form.dob);
    ...
    try {
      // Create a local receipt fingerprint. The actual ZK proof is generated by
      // proofProvider.proveTx during callTx.proveEligibility below.
      ...
      console.log('[4] Calling proveEligibility', { score, income, age });
      ...
      setStatus('success');
      setMessage(
        `Eligibility proof submitted successfully on Midnight. Transaction ID: ${txId}`
      );
      console.log('[10] Final success', { transactionId: txId });
      ...
      console.error('[10] Final error', err);
      setStatus('error');
